...
*   **Audio Recording:** Background audio recording with real-time volume bar visual feedback and automatic transcription. The volume bar is configurable via the Settings UI and can be seamlessly hidden to save space.
*   **Dual-Channel Audio Recording (System Loopback):** Simultaneously records your microphone and your computer's system audio (loopback), producing speaker-attributed transcriptions (differentiating between your voice and the system's output). The output text is visually differentiated with emojis (`🎤 ` for microphone, `💻 ` for system).
    *   **Windows**: Native support via WASAPI. Simply select your active speakers in the "System Loopback Device" dropdown in Settings.
    *   **macOS**: Requires a virtual audio driver (like BlackHole or Soundflower). You must route your system audio through the virtual driver and select it as the Loopback Device.
    *   **Linux**: Native support via PulseAudio/PipeWire. Select your default monitor or sink in the Loopback Device dropdown.
*   **Offline Speaker Diarization:** Processes recorded audio sessions through a dedicated FastAPI microservice leveraging `faster-whisper` and Pyannote's `whisperx`. Identifies distinct speakers within the audio, labels segments with their configured names, and saves the speaker-attributed transcript back to the session history. It dynamically utilizes CUDA for hardware acceleration when available.
...
*   **Real-time Speech Correction:** Mid-recording LLM feedback loop that analyzes transcribed speech through a rolling window of finalized sentences. Supports three independently toggleable correction types: **Fact-Checking** (flags incorrect statistics, dates, names), **Grammar & Pronunciation** (catches grammatical errors, malapropisms, transcription mistakes), and **Content Suggestions** (recommends stronger phrasing and supporting data). Grammar corrections can be powered by either the LLM or **LanguageTool** (local server or cloud Premium API) for faster, rule-based checking — the language is automatically mapped from the transcription language to LanguageTool's regional variant (e.g., `en` → `en-US`), with manual override available. Results are displayed in a draggable side panel (`CorrectionPanelWidget`) with colour-coded cards, collapsible explanations, and confidence indicators. The correction model is configurable per-profile (recommended: fast models like Flash Lite), the window size is adjustable, and correction prompts are user-editable in `config/prompts.json`. All corrections are stored in `session.json` under a `"corrections"` key. Configurable via the "Real-time Analysis" tab in the Settings UI.
...
*   **Webhooks / Integrations:** Configure an HTTP webhook URL to automatically trigger when a session ends or an audio summary is generated. Ideal for sending summaries to Slack, Zapier, or n8n. Webhooks can also be triggered manually for any session from the Session Browser.
...
*   **Modular Audio Architecture:** The audio recording system is decomposed into three focused modules: `whisperlive_manager.py` (service lifecycle — start, health check, cleanup), `audio_recorder.py` (device discovery, mic/loopback capture workers, volume metering, audio enhancement), and `transcription.py` (WhisperLive client management, real-time segment callbacks, utterance finalization, translation processing, Google STT fallback). `SoundSource` is a thin orchestrator that delegates to these modules.
*   **Service Availability Checks:** LLM engines expose an `is_available()` pre-flight check (Ollama, Antigravity). Pipeline handlers verify service reachability before execution, showing a clear error popup and aborting early if unavailable. Runtime service errors (WhisperLive, TTS, OCR, pipeline failures) are automatically surfaced as UI popups via a custom loguru sink that intercepts `ERROR+` messages from service-critical loggers.
*   **Advanced Logging (Loguru):** Logging uses loguru with colorised console output, rotating file logs (configurable path, rotation size, retention period, zip compression), and per-dependency log level overrides. A stdlib `InterceptHandler` bridges all existing `logging.getLogger()` calls to loguru with zero import changes. Console log level, file path, rotation/retention, and dependency levels are all configurable from the Settings UI.
