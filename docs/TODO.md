...
### Architecture
- [x] **Composable Pipeline Architecture** `[Pipecat]`: Refactored the monolithic `process_pipeline()` in `core/pipeline/pipeline.py` into a composable, frame-based pipeline inspired by Pipecat's processor architecture. Defined a `Processor` base class with a `process(frame) → frame` signature, frame types (`SourceFrame`, `PromptFrame`, `LLMResponseFrame`, `ErrorFrame`, `CancelledFrame`), and a `Pipeline` runner with `build_pipeline()` factory. New processing steps (PII filtering, translation, audio enhancement, routing) can be added by implementing a `Processor` subclass.
