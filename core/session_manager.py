...
class SessionManager:
    ...
    def start_new_session(self) -> str:
        ...
        if self.save_transcriptions:
            ...
            logger.debug(f"[SessionManager] Created new transcription file: {self.transcription_file}")
    ...
    def _load_session(self, session_id: str):
        ...
        if os.path.exists(path):
            ...
            if self.save_transcriptions:
                ...
                self.transcription_file = os.path.join(TRANSCRIPTIONS_DIR, filename)
                logger.debug(f"[SessionManager] Resumed session, created transcription file: {self.transcription_file}")
    ...
    def _load_last_session(self):
        ...
        if self.save_transcriptions:
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            filename = f"transcription_resumed_{self.current_session_id}_{timestamp}.txt"
            self.transcription_file = os.path.join(TRANSCRIPTIONS_DIR, filename)
            logger.debug(f"[SessionManager] Resumed last session, created transcription file: {self.transcription_file}")
    ...
    def append_interaction(
        ...
        if self.save_transcriptions and self.transcription_file and (source_name == "audio" or (extracted_text and "audio" in source_name.lower())):
    ...
    def append_transcription_segment(self, segment_text: str):
        ...
        if not self.save_transcriptions or not self.transcription_file or not segment_text:
            logger.debug(f"[SessionManager] Cannot append segment. save_trans={self.save_transcriptions}, file={self.transcription_file}, text='{segment_text}'")
            return
        ...
        try:
            ...
            logger.debug(f"[SessionManager] Appended segment to {self.transcription_file}: '{segment_text}'")
        except IOError as e:
            print(f"[SessionManager] Failed to append transcription segment to file: {e}")
            logger.error(f"[SessionManager] Failed to append transcription segment to file: {e}")

    def _save_transcription_to_file(self, prompt: str, response: str, extracted_text: Optional[str]):
        ...
        if not self.transcription_file:
            logger.debug(f"[SessionManager] Cannot save interaction. transcription_file is None.")
            return
        ...
        try:
            with open(self.transcription_file, "a", encoding="utf-8") as f:
                ...
                f.write(f"{response}\n\n--- User ---\n")
            logger.debug(f"[SessionManager] Saved AI response to {self.transcription_file}")
        except IOError as e:
    ...
    def _close_transcription_file(self):
        ...
        if self.transcription_file:
            logger.debug(f"[SessionManager] Closing transcription file tracking for: {self.transcription_file}")
        self.transcription_file = None
