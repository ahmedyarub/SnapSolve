...
class OllamaEngine(LLMEngine):
    def __init__(
        self, model: str, ollama_url: str, session_manager=None, status_callback=None
    ):
    ...
    def warmup(self, status_callback=None) -> bool:
        ...
        try:
            payload = {"model": self.model, "prompt": "Hello", "stream": False}
    ...
    def supports_images(self) -> bool:
        return (
            True  # Ollama generally supports images if the model does, we let it try.
        )
    ...
    def process_text(
        ...
        print(
            f"[OllamaEngine] Text Request started for model: {self.model} at {self.ollama_url}"
        )
        if status_callback:
    ...
    def process_image(
        ...
        print(
            f"[OllamaEngine] Image Request started for model: {self.model} at {self.ollama_url}"
        )
        if status_callback:
