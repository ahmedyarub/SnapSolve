ERR_SELECT_DEVICES = "Please select both input and output devices."
...
class SoundTestApp(QMainWindow):
    ...
    def play_audio(self, device_index, server_ready_event=None):
        try:
            device_name = self.p.get_device_info_by_index(device_index).get("name", UNKNOWN_DEVICE)
            self.signals.log_message.emit(f"Generating test audio for playback on {device_name}...")

            lines = [
                "This is a test of the WhisperLive transcription system.",
                "It should be able to transcribe my voice in real time.",
                "Let's see if it can handle this short sentence.",
                "And here is the final sentence for the test.",
            ]
            sample_rate = 22050

            wav_file = self._generate_test_audio(lines, sample_rate)
            if not wav_file:
                self.signals.log_message.emit("Failed to generate audio.")
                return
            ...
            self._play_wav_stream(wav_file, device_index)
    ...
    def _wait_for_whisperlive_ready(self, client):
        self.signals.log_message.emit("Waiting for WhisperLive server to be ready...")
        timeout, start_time = 15, time.time()

        while not client.recording:
            if client.server_error:
                self.signals.log_message.emit(f"WhisperLive server error: {getattr(client, 'error_message', 'Unknown error')}")
                try: client.close_websocket()
                except: pass
                break
            if client.waiting:
                self.signals.log_message.emit("WhisperLive server is full. Waiting for slot...")
            if time.time() - start_time > timeout:
                self.signals.log_message.emit("Timeout waiting for WhisperLive server.")
                try: client.close_websocket()
                except: pass
                break
            time.sleep(0.1)
    ...
    def _attempt_client_init(self, server_ready_event, attempt, max_retries, retry_delay):
        try:
            self.transcription_client = WhisperLiveTranscriptionClient(
                host="localhost", port=9090, lang="en", model="large-v3", use_vad=True,
                mute_audio_playback=True, enable_translation=False, display_segments=1,
                send_last_n_segments=3, transcription_callback=self.on_transcription_result,
                no_speech_thresh=0.9, clip_audio=False, same_output_threshold=5
            )

            client = self.transcription_client.client
            is_ready = self._wait_for_whisperlive_ready(client)

            if is_ready:
                self.signals.log_message.emit("WhisperLive server ready. Starting streaming...")
                if server_ready_event is not None: server_ready_event.set()
                return client

            if attempt < max_retries - 1:
                self.signals.log_message.emit(f"Connection attempt {attempt + 1} failed. Retrying...")
                time.sleep(retry_delay)
            else:
                self.signals.log_message.emit("Failed to connect after multiple attempts.")
                if server_ready_event is not None: server_ready_event.set()
                return None
        except Exception as e:
            self.signals.log_message.emit(f"Error initializing client (attempt {attempt + 1}): {e}")
            if attempt < max_retries - 1:
                time.sleep(retry_delay)
            else:
                self.signals.log_message.emit("Failed to init client after multiple attempts.")
                if server_ready_event is not None: server_ready_event.set()
                return None
        return None
    def _init_whisperlive_client(self, server_ready_event):
        max_retries = 3
        retry_delay = 2
        for attempt in range(max_retries):
            client = self._attempt_client_init(server_ready_event, attempt, max_retries, retry_delay)
            if client is not None:
                return client
        return None

    def _stream_microphone(self, device_index, client):
        with sr.Microphone(device_index=device_index) as source:
            stream = source.stream
            source_sample_rate = source.SAMPLE_RATE
            target_sample_rate = 16000

            self.signals.log_message.emit(f"Microphone sample rate: {source_sample_rate} Hz, Target: {target_sample_rate} Hz")
            resampler = resampy.resample if source_sample_rate != target_sample_rate else None
            ...
            while not self.playback_done:
                try:
                    ...
                    if resampler is not None:
                        audio_array = resampler(audio_array, source_sample_rate, target_sample_rate)
    ...
    def record_audio_to_file(self, device_index, server_ready_event=None):
        device_name = self.p.get_device_info_by_index(device_index).get("name", UNKNOWN_DEVICE)
        try:
            self.signals.log_message.emit(f"Starting recording on {device_name} for transcription...")
            self.signals.log_message.emit("Initializing WhisperLive client...")

            client = self._init_whisperlive_client(server_ready_event)
            if not client: return

            self._stream_microphone(device_index, client)
