...
## Testing
* Whenever we have a new class of one of the four pipeline components (Source, LLM Engine, Sink, OCR Engine), an additional e2e test should be created in `tests/test_e2e_gui.py`. We use `pytest-qt` to enforce realistic interactions via the GUI to ensure true end-to-end integration mapping over Qt contexts.
